...
## 2024-05-30 - [CRITICAL] Prevented IDOR on Profile Picture before 5-day limit
**Vulnerability:** The /user/{id} endpoint allowed matched users to fetch the unblurred profile image URL of their match immediately, bypassing the 5-day blurred period requirement.
**Learning:** Backend must enforce data masking (like image blurring). Returning the original URL and relying on frontend masking allows attackers to bypass the restriction via direct API calls.
**Prevention:** Apply data masking on the server side based on business rules (e.g., date checks) before returning the DTO/Entity to the client.
