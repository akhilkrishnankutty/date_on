...
## 2026-08-24 - [CRITICAL] Prevented IDOR on Profile Pictures for Matches
**Vulnerability:** The `/user/{id}` endpoint exposed unblurred profile picture URLs for matched users before the 5-day mandatory period expired, relying on the frontend to mask it.
**Learning:** Never trust the frontend to mask or hide sensitive data, and do not use client-side manipulated URL parameters (like Cloudinary's `e_blur`) for security. Attackers can easily strip them.
**Prevention:** Apply backend-only blurring by returning a hardcoded, generic blurred placeholder URL for sensitive images until conditions (like match duration) are fully met.
