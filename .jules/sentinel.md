...
## 2026-05-02 - [CRITICAL] Removed Hardcoded Secrets
**Vulnerability:** Cloudinary API keys and secrets were hardcoded directly in `application.properties`, exposing them to anyone with repository access.
**Learning:** Hardcoded credentials should be replaced with environment variable substitutions without default fallbacks containing the actual secrets.
**Prevention:** Use environment variables (`${CLOUDINARY_API_KEY:your_api_key_here}`, etc.) to allow secure injection while maintaining safe local fallbacks.
