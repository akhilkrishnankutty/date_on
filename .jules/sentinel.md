...
## 2026-08-06 - [CRITICAL] Prevented IDOR on Profile Picture Exposure
**Vulnerability:** The `/user/{id}` endpoint returned the raw profile picture URL to matched users, relying on the frontend to blur the image if the match was less than 5 days old. This allowed an attacker to intercept the response and view unblurred images via IDOR.
**Learning:** Never trust the frontend to mask or hide sensitive data. All data filtering must occur on the backend.
**Prevention:** Apply backend checks (e.g., verifying the `matchTime`) and replace sensitive data with generic placeholders (like a hardcoded blurred avatar URL) before returning it in the API response.
