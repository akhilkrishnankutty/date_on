...
public class UserController {
    ...
    public ResponseEntity<?> getUser(
        ...
        if ("MATCHED".equals(currentUser.getStatus()) && currentUser.getLoid() == user.getId()) {
            ...
            if (user.getMatchTime() != null && user.getMatchTime().isBefore(java.time.LocalDateTime.now().minusDays(5))) {
                safeUser.setProfilePictureUrl(user.getProfilePictureUrl());
            } else {
                safeUser.setProfilePictureUrl("https://res.cloudinary.com/generic-domain/image/upload/v123456789/placeholder_blur.jpg");
            }
