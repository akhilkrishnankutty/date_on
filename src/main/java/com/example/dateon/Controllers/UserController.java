...
public class UserController {
    ...
    public ResponseEntity<?> getUser(
        ...
        if ("MATCHED".equals(currentUser.getStatus()) && currentUser.getLoid() == user.getId()) {
            ...
            if (user.getMatchTime() != null && java.time.LocalDateTime.now().isAfter(user.getMatchTime().plusDays(5))) {
                safeUser.setProfilePictureUrl(user.getProfilePictureUrl());
            } else {
                safeUser.setProfilePictureUrl("https://res.cloudinary.com/dquj7szs3/image/upload/v1726000000/blurred_placeholder.jpg");
            }
